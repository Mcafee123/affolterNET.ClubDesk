...
get_serviceprincipal() {
    sp_name=${1:-$sp_devops_name}
    service_principal=$(az ad sp list --all --query "[?appDisplayName == '$sp_name']")
...
create_serviceprincipal() {
    ...
    scopes=${3:-"/subscriptions/$subscriptionid/resourceGroups/$resourceGroup"}
    service_principal=$(get_serviceprincipal $sp_name)
    ...
    echo
    echo "create __${env}_${sp_name}.sh"
    echo "export SP_DEVOPS_ID=\"$appId\"" > __${env}_${sp_name}.sh
    echo "export SP_TENANT_ID=\"$tenant_id\"" >> __${env}_${sp_name}.sh
    echo "export SP_DEVOPS_PASSWORD"=\"$client_secret\" >> __${env}_${sp_name}.sh
    echo
}
